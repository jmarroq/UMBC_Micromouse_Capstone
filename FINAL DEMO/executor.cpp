...
static const float CELL_MM_BASE = 175.0f;
static const float TURN_TICKS_90  = 70.0f;
static const uint16_t FRONT_APPROACH_TARGET_MM = 40;
static const uint16_t FRONT_APPROACH_ENABLE_MM = 200;
...
static const uint8_t LONG_RUN_THRESHOLD = 2;
static const float MM_PER_TICK    = 1.0f;     
static const float TURN_TICKS_180 = 2.15f * TURN_TICKS_90;
...
static bool alignOnlyMode = false;
    ...
    case STEP_ALIGN: {
      ...
      if (turned >= activeTurnTargetTicks) {
        ...
        if (alignOnlyMode) {
          ...
          Serial.println("EXEC ALIGN ONLY COMPLETE");
        } else {
          currentStepPhase = STEP_FORWARD;
          startPrimitive(PRIM_FORWARD_ONE_CELL);
...
void executorBeginAlignOnly(Direction targetDir) {
  ...
  if (firstPrim == PRIM_FORWARD_ONE_CELL) {
    ...
    stepDoneFlag = true;
    alignOnlyMode = false;
    Serial.println("EXEC ALIGN ONLY COMPLETE - ALREADY ALIGNED");
    return;
  }
  ...
  Serial.println("EXEC START ALIGN ONLY");
}
...
bool executorBusy() {
  return currentStepPhase != STEP_IDLE;
}